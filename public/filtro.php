...
<head>
    ...
    <style>
        ...
        .filter-group label {
            display: block; font-weight: bold; font-size: 0.8rem;
            margin-bottom: 8px; color: var(--marron-oscuro); text-transform: uppercase;
        }

        select, input[type="text"], input[type="range"] { width: 100%; padding: 8px; border-radius: 4px; border: 1px solid var(--gris-claro); }
        ...
        .btn-apply {
            width: 100%; padding: 12px; background: var(--marron-claro);
        ...
        .producto-card { background: #fff; border: 1px solid #eee; padding: 15px; text-align: center; border-radius: 8px; transition: 0.3s; }
        .producto-card:hover { box-shadow: 0 5px 15px rgba(0,0,0,0.1); }
        .prod-img { background: var(--gris-claro); height: 180px; margin-bottom: 10px; border-radius: 4px; display: flex; align-items: center; justify-content: center; color: #888; }
    </style>
</head>
<body>
    ...
    <aside class="sidebar">
        <form method="GET" action="">
                ...
                <div class="color-grid">
                    <?php foreach($filtros_data['colores'] as $nombre => $hex): ?>
                        <label class="color-circle">
                            ...
                            <span class="inner" style="background-color: <?php echo $hex; ?>;" title="<?php echo $nombre; ?>"></span>
                        </label>
                    <?php endforeach; ?>
                </div>
            </div>
            ...
            <div class="filter-group">
                ...
                <input type="range" name="precio" min="0" max="500" value="<?php echo $_GET['precio'] ?? '500'; ?>" id="range-p">
            </div>

            <button type="submit" class="btn-apply">APLICAR FILTROS</button>
            <a href="filtros.php" style="display:block; text-align:center; margin-top:10px; color:var(--gris-medio); font-size:0.8rem; text-decoration:none;">Limpiar</a>
        </form>
    </aside>
            ...
            <?php if ($resultado->num_rows > 0): ?>
                <?php while($prod = $resultado->fetch_assoc()): ?>
                    <div class="producto-card">
                        ...
                        <div style="color:var(--marron-claro); font-weight:bold;"><?php echo number_format($prod['precio'], 2); ?>€</div>
                        <div style="font-size:0.7rem; color:#999;"><?php echo $prod['color']; ?> | <?php echo $prod['categoria']; ?></div>
                    </div>
                <?php endwhile; ?>
            <?php else: ?>
